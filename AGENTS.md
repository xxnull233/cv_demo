...
如果全局 `codegraph` 命令因权限不足报错 `EPERM`，通过 `npx` 调用项目本地安装的版本即可：

```bash
npx -p @colbymchenry/codegraph codegraph <subcommand>
```

项目已预置快捷 npm scripts（免去每次写 `npx -p ...`）：

| 命令 | 等价操作 |
|---|---|
| `npm run cg:status` | 查看索引状态 |
| `npm run cg:sync` | 同步索引（修改/删除文件后执行） |
| `npm run cg:search -- <query>` | 搜索符号 |
| `npm run cg:explore -- <symbols...>` | 查看多个符号源码 |
| `npm run cg:context` | 获取任务相关上下文 |
